...
impl TryFrom<u32> for PetitionTypeKey {
    type Error = u32;
...
impl TryFrom<usize> for PetitionTypeKey {
    type Error = usize;
...
impl TryFrom<u64> for PetitionTypeKey {
    type Error = u64;
...
impl TryFrom<i64> for PetitionTypeKey {
    type Error = i64;
...
impl TryFrom<isize> for PetitionTypeKey {
    type Error = isize;
...
mod test {
    ...
    fn petition_type() {
        let mut file = File::open("../wrath-dbc/PetitionType.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = PetitionType::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = PetitionType::read(&mut v.as_slice()).unwrap();
