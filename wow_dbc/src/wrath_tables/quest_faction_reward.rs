...
impl TryFrom<u32> for QuestFactionRewardKey {
    type Error = u32;
...
impl TryFrom<usize> for QuestFactionRewardKey {
    type Error = usize;
...
impl TryFrom<u64> for QuestFactionRewardKey {
    type Error = u64;
...
impl TryFrom<i64> for QuestFactionRewardKey {
    type Error = i64;
...
impl TryFrom<isize> for QuestFactionRewardKey {
    type Error = isize;
...
mod test {
    ...
    fn quest_faction_reward() {
        let mut file = File::open("../wrath-dbc/QuestFactionReward.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = QuestFactionReward::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = QuestFactionReward::read(&mut v.as_slice()).unwrap();
