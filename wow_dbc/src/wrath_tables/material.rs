...
impl TryFrom<u32> for MaterialKey {
    type Error = u32;
...
mod test {
    ...
    fn material() {
        let mut file = File::open("../wrath-dbc/Material.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = Material::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = Material::read(&mut v.as_slice()).unwrap();
