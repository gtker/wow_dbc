...
impl DbcTable for Languages {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (Languages) int32
            let id = LanguagesKey::new(crate::util::read_i32_le(chunk)?);
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstLanguages<const S: usize> {
    pub rows: [ConstLanguagesRow; S],
}

impl<const S: usize> ConstLanguages<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (Languages) int32
            let id = LanguagesKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
    ...
    pub fn to_owned(&self) -> Languages {
        Languages {
            rows: self.rows.iter().map(|s| LanguagesRow {
                id: s.id,
