...
impl DbcTable for GMSurveyQuestions {
    ...
    fn write(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // question_lang: string_ref_loc (Extended)
            b.write_all(&row.question_lang.string_indices_as_array(&mut string_index))?;
...
impl GMSurveyQuestions {
    fn write_string_block(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            row.question_lang.string_block_as_array(b)?;
        }
    ...
    fn string_block_size(&self) -> u32 {
        let mut sum = 1;
        for row in &self.rows {
            sum += row.question_lang.string_block_size();
        }
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstGMSurveyQuestions<const S: usize> {
    pub rows: [ConstGMSurveyQuestionsRow; S],
}

impl<const S: usize> ConstGMSurveyQuestions<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut rows = [
            ConstGMSurveyQuestionsRow {
                id: GMSurveyQuestionsKey::new(0),
                question_lang: crate::ConstExtendedLocalizedString::empty(),
            }
        ; S];
        ...
        while i < S {
            // id: primary_key (GMSurveyQuestions) int32
            let id = GMSurveyQuestionsKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // question_lang: string_ref_loc (Extended)
            let question_lang = ConstExtendedLocalizedString::new(
            ...
            rows[i] = ConstGMSurveyQuestionsRow {
                id,
                question_lang,
            };
            i += 1;
    ...
    pub fn to_owned(&self) -> GMSurveyQuestions {
        GMSurveyQuestions {
            rows: self.rows.iter().map(|s| GMSurveyQuestionsRow {
                id: s.id,
                question_lang: s.question_lang.to_string(),
            }).collect(),
        }
    }
    // TODO: Indexable?
...
pub struct ConstGMSurveyQuestionsRow {
    pub id: GMSurveyQuestionsKey,
    pub question_lang: ConstExtendedLocalizedString,
}
