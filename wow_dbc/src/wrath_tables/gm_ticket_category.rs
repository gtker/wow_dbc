...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstGMTicketCategory<const S: usize> {
    pub rows: [ConstGMTicketCategoryRow; S],
}

impl<const S: usize> ConstGMTicketCategory<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut b_offset = 20;
        let mut rows = [
            ConstGMTicketCategoryRow {
                id: GMTicketCategoryKey::new(0),
                category_lang: crate::ConstExtendedLocalizedString::empty(),
            }
        ; S];
        ...
        while i < S {
            // id: primary_key (GMTicketCategory) int32
            let id = GMTicketCategoryKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // category_lang: string_ref_loc (Extended)
            let category_lang = ConstExtendedLocalizedString::new(
            ...
            rows[i] = ConstGMTicketCategoryRow {
                id,
                category_lang,
            };
            i += 1;
...
pub struct ConstGMTicketCategoryRow {
    pub id: GMTicketCategoryKey,
    pub category_lang: ConstExtendedLocalizedString,
}
