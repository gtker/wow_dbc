...
impl DbcTable for GMTicketCategory {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // category_lang: string_ref_loc (Extended)
            b.write_all(&row.category_lang.string_indices_as_array(&mut string_cache))?;
...
mod test {
    ...
    fn gm_ticket_category() {
        let mut file = File::open("../wrath-dbc/GMTicketCategory.dbc").expect("Failed to open DBC file");
