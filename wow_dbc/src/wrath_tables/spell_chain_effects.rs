...
impl DbcTable for SpellChainEffects {
    ...
    const FIELD_COUNT: usize = 48;
    const ROW_SIZE: usize = 177;
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // combo: string_ref
            b.write_all(&string_cache.add_string(&row.combo).to_le_bytes())?;
...
mod test {
    ...
    fn spell_chain_effects() {
        let mut file = File::open("../wrath-dbc/SpellChainEffects.dbc").expect("Failed to open DBC file");
