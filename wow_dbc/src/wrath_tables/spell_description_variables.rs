...
impl DbcTable for SpellDescriptionVariables {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            let id = SpellDescriptionVariablesKey::new(crate::util::read_i32_le(chunk)?);
            ...
            rows.push(SpellDescriptionVariablesRow {
                id,
                variables,
            });
        }

        Ok(SpellDescriptionVariables { rows, })
    }
    ...
    fn write(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            if !row.variables.is_empty() {
                ...
                string_index += row.variables.len() + 1;
            }
            else {
...
impl SpellDescriptionVariables {
    fn write_string_block(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            if !row.variables.is_empty() { b.write_all(row.variables.as_bytes())?; b.write_all(&[0])?; };
        }
    ...
    fn string_block_size(&self) -> u32 {
        let mut sum = 1;
        for row in &self.rows {
            if !row.variables.is_empty() { sum += row.variables.len() + 1; };
        }
...
mod test {
    ...
    fn spell_description_variables() {
        let mut file = File::open("../wrath-dbc/SpellDescriptionVariables.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = SpellDescriptionVariables::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = SpellDescriptionVariables::read(&mut v.as_slice()).unwrap();
