...
pub struct ConstDungeonMapChunk<const S: usize> {
    pub rows: [DungeonMapChunkRow; S],
}

impl<const S: usize> ConstDungeonMapChunk<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut rows = [
            DungeonMapChunkRow {
                ...
                map_id: MapKey::new(0),
                w_m_o_group_id: WMOAreaTableKey::new(0),
                dungeon_map_id: DungeonMapKey::new(0),
                min_z: 0.0,
            }
        ; S];
        ...
        while i < S {
            // id: primary_key (DungeonMapChunk) int32
            let id = DungeonMapChunkKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
            ...
            // w_m_o_group_id: foreign_key (WMOAreaTable) int32
            let w_m_o_group_id = WMOAreaTableKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // dungeon_map_id: foreign_key (DungeonMap) int32
            let dungeon_map_id = DungeonMapKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // min_z: float
            let min_z = crate::util::ct_u32_to_f32([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]);
            b_offset += 4;

            rows[i] = DungeonMapChunkRow {
                id,
                map_id,
                w_m_o_group_id,
                dungeon_map_id,
