...
impl DbcTable for SpellRadius {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (SpellRadius) int32
            let id = SpellRadiusKey::new(crate::util::read_i32_le(chunk)?);
...
impl From<i8> for SpellRadiusKey {
    fn from(v: i8) -> Self {
        Self::new(v.into())
    }
}

impl From<i16> for SpellRadiusKey {
    fn from(v: i16) -> Self {
        Self::new(v.into())
    }
}

impl From<i32> for SpellRadiusKey {
    fn from(v: i32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u32> for SpellRadiusKey {
    type Error = u32;
...
impl TryFrom<usize> for SpellRadiusKey {
    type Error = usize;
...
impl TryFrom<u64> for SpellRadiusKey {
    type Error = u64;
...
impl TryFrom<i64> for SpellRadiusKey {
    type Error = i64;
...
impl TryFrom<isize> for SpellRadiusKey {
    type Error = isize;
...
mod test {
    ...
    fn spell_radius() {
        let mut file = File::open("../wrath-dbc/SpellRadius.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = SpellRadius::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = SpellRadius::read(&mut v.as_slice()).unwrap();
