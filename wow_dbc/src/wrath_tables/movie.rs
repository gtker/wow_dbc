...
impl TryFrom<usize> for MovieKey {
    type Error = usize;
...
impl TryFrom<isize> for MovieKey {
    type Error = isize;
...
mod test {
    ...
    fn movie() {
        let mut file = File::open("../wrath-dbc/Movie.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = Movie::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = Movie::read(&mut v.as_slice()).unwrap();
