...
impl TryFrom<u32> for SpellDifficultyKey {
    type Error = u32;
...
impl TryFrom<usize> for SpellDifficultyKey {
    type Error = usize;
...
impl TryFrom<u64> for SpellDifficultyKey {
    type Error = u64;
...
impl TryFrom<i64> for SpellDifficultyKey {
    type Error = i64;
...
impl TryFrom<isize> for SpellDifficultyKey {
    type Error = isize;
...
mod test {
    ...
    fn spell_difficulty() {
        let mut file = File::open("../wrath-dbc/SpellDifficulty.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = SpellDifficulty::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = SpellDifficulty::read(&mut v.as_slice()).unwrap();
