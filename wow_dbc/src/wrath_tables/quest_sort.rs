...
impl DbcTable for QuestSort {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // sort_name_lang: string_ref_loc (Extended)
            b.write_all(&row.sort_name_lang.string_indices_as_array(&mut string_cache))?;
...
impl TryFrom<u32> for QuestSortKey {
    type Error = u32;
...
mod test {
    ...
    fn quest_sort() {
        let mut file = File::open("../wrath-dbc/QuestSort.dbc").expect("Failed to open DBC file");
