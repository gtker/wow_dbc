...
pub struct ConstGlyphProperties<const S: usize> {
    pub rows: [GlyphPropertiesRow; S],
}

impl<const S: usize> ConstGlyphProperties<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (GlyphProperties) int32
            let id = GlyphPropertiesKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
            ...
            // glyph_slot_flags: int32
            let glyph_slot_flags = i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]);
            b_offset += 4;
