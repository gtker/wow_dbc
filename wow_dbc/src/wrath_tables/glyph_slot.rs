...
mod test {
    ...
    fn glyph_slot() {
        let mut file = File::open("../wrath-dbc/GlyphSlot.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = GlyphSlot::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = GlyphSlot::read(&mut v.as_slice()).unwrap();
