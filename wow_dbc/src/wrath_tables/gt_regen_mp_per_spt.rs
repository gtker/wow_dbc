...
impl DbcTable for gtRegenMPPerSpt {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            rows.push(gtRegenMPPerSptRow {
                data,
            });
        }

        Ok(gtRegenMPPerSpt { rows, })
    }
...
pub struct ConstgtRegenMPPerSpt<const S: usize> {
    pub rows: [gtRegenMPPerSptRow; S],
}

impl<const S: usize> ConstgtRegenMPPerSpt<S> {
    ...
    pub fn to_owned(&self) -> gtRegenMPPerSpt {
        gtRegenMPPerSpt {
            rows: self.rows.iter().map(|s| gtRegenMPPerSptRow {
                data: s.data,
            }).collect(),
        }
    }
