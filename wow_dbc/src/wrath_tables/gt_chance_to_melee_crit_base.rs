...
mod test {
    ...
    fn gt_chance_to_melee_crit_base() {
        let mut file = File::open("../wrath-dbc/gtChanceToMeleeCritBase.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = gtChanceToMeleeCritBase::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = gtChanceToMeleeCritBase::read(&mut v.as_slice()).unwrap();
