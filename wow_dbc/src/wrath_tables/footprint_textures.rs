...
impl DbcTable for FootprintTextures {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // footstep_filename: string_ref
            b.write_all(&string_cache.add_string(&row.footstep_filename).to_le_bytes())?;
...
impl TryFrom<u32> for FootprintTexturesKey {
    type Error = u32;
...
mod test {
    ...
    fn footprint_textures() {
        let mut file = File::open("../wrath-dbc/FootprintTextures.dbc").expect("Failed to open DBC file");
