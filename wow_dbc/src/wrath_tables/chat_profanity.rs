...
impl DbcTable for ChatProfanity {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (ChatProfanity) int32
            let id = ChatProfanityKey::new(crate::util::read_i32_le(chunk)?);
...
impl From<i8> for ChatProfanityKey {
    fn from(v: i8) -> Self {
        Self::new(v.into())
    }
}

impl From<i16> for ChatProfanityKey {
    fn from(v: i16) -> Self {
        Self::new(v.into())
    }
}

impl From<i32> for ChatProfanityKey {
    fn from(v: i32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u32> for ChatProfanityKey {
    type Error = u32;
...
impl TryFrom<usize> for ChatProfanityKey {
    type Error = usize;
...
impl TryFrom<u64> for ChatProfanityKey {
    type Error = u64;
...
impl TryFrom<i64> for ChatProfanityKey {
    type Error = i64;
...
impl TryFrom<isize> for ChatProfanityKey {
    type Error = isize;
...
mod test {
    ...
    fn chat_profanity() {
        let mut file = File::open("../wrath-dbc/ChatProfanity.dbc").expect("Failed to open DBC file");
