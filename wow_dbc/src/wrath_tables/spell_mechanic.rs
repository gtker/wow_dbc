...
impl DbcTable for SpellMechanic {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // state_name_lang: string_ref_loc (Extended)
            b.write_all(&row.state_name_lang.string_indices_as_array(&mut string_cache))?;
...
mod test {
    ...
    fn spell_mechanic() {
        let mut file = File::open("../wrath-dbc/SpellMechanic.dbc").expect("Failed to open DBC file");
