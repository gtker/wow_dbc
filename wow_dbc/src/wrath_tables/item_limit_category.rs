...
impl TryFrom<u32> for ItemLimitCategoryKey {
    type Error = u32;
...
impl TryFrom<usize> for ItemLimitCategoryKey {
    type Error = usize;
...
impl TryFrom<u64> for ItemLimitCategoryKey {
    type Error = u64;
...
impl TryFrom<i64> for ItemLimitCategoryKey {
    type Error = i64;
...
impl TryFrom<isize> for ItemLimitCategoryKey {
    type Error = isize;
...
mod test {
    ...
    fn item_limit_category() {
        let mut file = File::open("../wrath-dbc/ItemLimitCategory.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = ItemLimitCategory::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = ItemLimitCategory::read(&mut v.as_slice()).unwrap();
