...
impl DbcTable for CreatureType {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (CreatureType) int32
            let id = CreatureTypeKey::new(crate::util::read_i32_le(chunk)?);
...
impl From<i8> for CreatureTypeKey {
    fn from(v: i8) -> Self {
        Self::new(v.into())
    }
}

impl From<i16> for CreatureTypeKey {
    fn from(v: i16) -> Self {
        Self::new(v.into())
    }
}

impl From<i32> for CreatureTypeKey {
    fn from(v: i32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u32> for CreatureTypeKey {
    type Error = u32;
...
impl TryFrom<usize> for CreatureTypeKey {
    type Error = usize;
...
impl TryFrom<u64> for CreatureTypeKey {
    type Error = u64;
...
impl TryFrom<i64> for CreatureTypeKey {
    type Error = i64;
...
impl TryFrom<isize> for CreatureTypeKey {
    type Error = isize;
...
mod test {
    ...
    fn creature_type() {
        let mut file = File::open("../wrath-dbc/CreatureType.dbc").expect("Failed to open DBC file");
