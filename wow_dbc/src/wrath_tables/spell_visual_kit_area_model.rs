...
mod test {
    ...
    fn spell_visual_kit_area_model() {
        let mut file = File::open("../wrath-dbc/SpellVisualKitAreaModel.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = SpellVisualKitAreaModel::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = SpellVisualKitAreaModel::read(&mut v.as_slice()).unwrap();
