...
impl<const S: usize> ConstCharHairGeosets<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (CharHairGeosets) int32
            let id = CharHairGeosetsKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
            ...
            // geoset_id: int32
            let geoset_id = i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]);
            b_offset += 4;

            // showscalp: int32
            let showscalp = i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]);
            b_offset += 4;
    ...
    pub fn to_owned(&self) -> CharHairGeosets {
        CharHairGeosets {
            rows: self.rows.iter().map(|s| CharHairGeosetsRow {
                ...
                geoset_id: s.geoset_id,
                showscalp: s.showscalp,
            }).collect(),
        }
    }
    // TODO: Indexable?
