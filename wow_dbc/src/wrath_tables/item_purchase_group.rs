...
impl TryFrom<usize> for ItemPurchaseGroupKey {
    type Error = usize;
...
impl TryFrom<u64> for ItemPurchaseGroupKey {
    type Error = u64;
...
impl TryFrom<i64> for ItemPurchaseGroupKey {
    type Error = i64;
...
impl TryFrom<isize> for ItemPurchaseGroupKey {
    type Error = isize;
...
mod test {
    ...
    fn item_purchase_group() {
        let mut file = File::open("../wrath-dbc/ItemPurchaseGroup.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = ItemPurchaseGroup::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = ItemPurchaseGroup::read(&mut v.as_slice()).unwrap();
