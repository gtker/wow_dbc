...
impl DbcTable for CreatureFamily {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // icon_file: string_ref
            b.write_all(&string_cache.add_string(&row.icon_file).to_le_bytes())?;
...
impl TryFrom<u32> for CreatureFamilyKey {
    type Error = u32;
...
mod test {
    ...
    fn creature_family() {
        let mut file = File::open("../wrath-dbc/CreatureFamily.dbc").expect("Failed to open DBC file");
