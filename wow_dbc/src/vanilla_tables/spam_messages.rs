...
impl DbcTable for SpamMessages {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (SpamMessages) uint32
            let id = SpamMessagesKey::new(crate::util::read_u32_le(chunk)?);
...
impl From<u32> for SpamMessagesKey {
    fn from(v: u32) -> Self {
        Self::new(v)
    }
}
...
impl TryFrom<i8> for SpamMessagesKey {
    type Error = i8;
...
impl TryFrom<i16> for SpamMessagesKey {
    type Error = i16;
...
impl TryFrom<i32> for SpamMessagesKey {
    type Error = i32;
...
mod test {
    ...
    fn spam_messages() {
        let mut file = File::open("../vanilla-dbc/SpamMessages.dbc").expect("Failed to open DBC file");
