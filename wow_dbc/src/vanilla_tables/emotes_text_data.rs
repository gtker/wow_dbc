...
impl DbcTable for EmotesTextData {
    ...
    fn filename() -> &'static str { "EmotesTextData.dbc" }
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (EmotesTextData) uint32
            let id = EmotesTextDataKey::new(crate::util::read_u32_le(chunk)?);
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstEmotesTextData<const S: usize> {
    pub rows: [ConstEmotesTextDataRow; S],
}

impl<const S: usize> ConstEmotesTextData<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut rows = [
            ConstEmotesTextDataRow {
                id: EmotesTextDataKey::new(0),
        ...
        while i < S {
            // id: primary_key (EmotesTextData) uint32
            let id = EmotesTextDataKey::new(u32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
    ...
    pub fn to_owned(&self) -> EmotesTextData {
        EmotesTextData {
            rows: self.rows.iter().map(|s| EmotesTextDataRow {
                id: s.id,
...
pub struct ConstEmotesTextDataRow {
    pub id: EmotesTextDataKey,
