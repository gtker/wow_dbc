...
impl DbcTable for NameGen {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            let id = NameGenKey::new(crate::util::read_u32_le(chunk)?);
...
impl TryFrom<i8> for NameGenKey {
    type Error = i8;
...
impl TryFrom<i16> for NameGenKey {
    type Error = i16;
...
impl TryFrom<i32> for NameGenKey {
    type Error = i32;
...
mod test {
    ...
    fn name_gen() {
        let mut file = File::open("../vanilla-dbc/NameGen.dbc").expect("Failed to open DBC file");
        let mut contents = Vec::new();
        file.read_to_end(&mut contents).expect("Failed to read DBC file");
        let actual = NameGen::read(&mut contents.as_slice()).unwrap();
        let mut v = Vec::with_capacity(contents.len());
        actual.write(&mut v).unwrap();
        let new = NameGen::read(&mut v.as_slice()).unwrap();
