...
impl DbcTable for GameTips {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (GameTips) uint32
            let id = GameTipsKey::new(crate::util::read_u32_le(chunk)?);
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstGameTips<const S: usize> {
    pub rows: [ConstGameTipsRow; S],
}

impl<const S: usize> ConstGameTips<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (GameTips) uint32
            let id = GameTipsKey::new(u32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
