...
impl DbcTable for MailTemplate {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // body: string_ref_loc
            b.write_all(&row.body.string_indices_as_array(&mut string_cache))?;
...
impl TryFrom<i8> for MailTemplateKey {
    type Error = i8;
...
impl TryFrom<i16> for MailTemplateKey {
    type Error = i16;
...
impl TryFrom<i32> for MailTemplateKey {
    type Error = i32;
...
mod test {
    ...
    fn mail_template() {
        let mut file = File::open("../vanilla-dbc/MailTemplate.dbc").expect("Failed to open DBC file");
