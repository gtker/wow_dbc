...
impl DbcTable for SpellMechanic {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // state_name: string_ref_loc
            b.write_all(&row.state_name.string_indices_as_array(&mut string_cache))?;
...
impl TryFrom<i8> for SpellMechanicKey {
    type Error = i8;
...
impl TryFrom<i16> for SpellMechanicKey {
    type Error = i16;
...
impl TryFrom<i32> for SpellMechanicKey {
    type Error = i32;
...
mod test {
    ...
    fn spell_mechanic() {
        let mut file = File::open("../vanilla-dbc/SpellMechanic.dbc").expect("Failed to open DBC file");
