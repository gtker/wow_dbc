...
impl DbcTable for Stationery {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (Stationery) uint32
            let id = StationeryKey::new(crate::util::read_u32_le(chunk)?);

            // item: uint32
            let item = crate::util::read_u32_le(chunk)?;
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // item: uint32
            b.write_all(&row.item.to_le_bytes())?;
...
impl TryFrom<i8> for StationeryKey {
    type Error = i8;
...
impl TryFrom<i16> for StationeryKey {
    type Error = i16;
...
impl TryFrom<i32> for StationeryKey {
    type Error = i32;
...
mod test {
    ...
    fn stationery() {
        let mut file = File::open("../vanilla-dbc/Stationery.dbc").expect("Failed to open DBC file");
