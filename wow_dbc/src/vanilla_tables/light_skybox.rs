...
impl DbcTable for LightSkybox {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // skybox_model_path: string_ref
            b.write_all(&string_cache.add_string(&row.skybox_model_path).to_le_bytes())?;
...
impl From<u32> for LightSkyboxKey {
    fn from(v: u32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u64> for LightSkyboxKey {
    type Error = u64;
...
impl TryFrom<usize> for LightSkyboxKey {
    type Error = usize;
...
impl TryFrom<i8> for LightSkyboxKey {
    type Error = i8;
...
impl TryFrom<i16> for LightSkyboxKey {
    type Error = i16;
...
impl TryFrom<i32> for LightSkyboxKey {
    type Error = i32;
...
impl TryFrom<i64> for LightSkyboxKey {
    type Error = i64;
...
impl TryFrom<isize> for LightSkyboxKey {
    type Error = isize;
...
mod test {
    ...
    fn light_skybox() {
        let mut file = File::open("../vanilla-dbc/LightSkybox.dbc").expect("Failed to open DBC file");
