...
impl DbcTable for StringLookups {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // path: string_ref
            b.write_all(&string_cache.add_string(&row.path).to_le_bytes())?;
...
impl From<u32> for StringLookupsKey {
    fn from(v: u32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u64> for StringLookupsKey {
    type Error = u64;
...
impl TryFrom<usize> for StringLookupsKey {
    type Error = usize;
...
impl TryFrom<i8> for StringLookupsKey {
    type Error = i8;
...
impl TryFrom<i16> for StringLookupsKey {
    type Error = i16;
...
impl TryFrom<i32> for StringLookupsKey {
    type Error = i32;
...
impl TryFrom<i64> for StringLookupsKey {
    type Error = i64;
...
impl TryFrom<isize> for StringLookupsKey {
    type Error = isize;
...
mod test {
    ...
    fn string_lookups() {
        let mut file = File::open("../vanilla-dbc/StringLookups.dbc").expect("Failed to open DBC file");
