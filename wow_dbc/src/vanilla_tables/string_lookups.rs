...
impl<const S: usize> ConstStringLookups<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (StringLookups) uint32
            let id = StringLookupsKey::new(u32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // path: string_ref
            let path = crate::util::get_string_from_block(b_offset, b, string_block);
            b_offset += 4;
