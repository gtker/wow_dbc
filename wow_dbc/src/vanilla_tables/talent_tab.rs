...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstTalentTab<const S: usize> {
    pub rows: [ConstTalentTabRow; S],
}

impl<const S: usize> ConstTalentTab<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        while i < S {
            // id: primary_key (TalentTab) uint32
            let id = TalentTabKey::new(u32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
            ...
            // order_index: uint32
            let order_index = u32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]);
            b_offset += 4;

            // background_file: string_ref
            let background_file = crate::util::get_string_from_block(b_offset, b, string_block);
            b_offset += 4;

            rows[i] = ConstTalentTabRow {
                id,
                name,
                spell_icon,
                race_mask,
...
pub struct ConstTalentTabRow {
    ...
    pub class_mask: ChrClassesKey,
    pub order_index: u32,
    pub background_file: &'static str,
}
