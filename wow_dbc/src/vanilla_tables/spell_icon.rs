...
impl DbcTable for SpellIcon {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // texture_file: string_ref
            b.write_all(&string_cache.add_string(&row.texture_file).to_le_bytes())?;
...
impl TryFrom<i8> for SpellIconKey {
    type Error = i8;
...
impl TryFrom<i16> for SpellIconKey {
    type Error = i16;
...
impl TryFrom<i32> for SpellIconKey {
    type Error = i32;
...
mod test {
    ...
    fn spell_icon() {
        let mut file = File::open("../vanilla-dbc/SpellIcon.dbc").expect("Failed to open DBC file");
