...
impl DbcTable for GroundEffectDoodad {
    ...
    fn write(&self, w: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // doodad_path: string_ref
            b.write_all(&string_cache.add_string(&row.doodad_path).to_le_bytes())?;
...
mod test {
    ...
    fn ground_effect_doodad() {
        let mut file = File::open("../vanilla-dbc/GroundEffectDoodad.dbc").expect("Failed to open DBC file");
