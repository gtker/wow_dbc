...
impl<const S: usize> ConstTaxiPath<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut rows = [
            TaxiPathRow {
                ...
                from_taxi_node: TaxiNodesKey::new(0),
                to_taxi_node: TaxiNodesKey::new(0),
                cost: 0,
            }
        ; S];
        ...
        while i < S {
            // id: primary_key (TaxiPath) int32
            let id = TaxiPathKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // from_taxi_node: foreign_key (TaxiNodes) int32
            let from_taxi_node = TaxiNodesKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // to_taxi_node: foreign_key (TaxiNodes) int32
            let to_taxi_node = TaxiNodesKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;
    ...
    pub fn to_owned(&self) -> TaxiPath {
        TaxiPath {
            rows: self.rows.iter().map(|s| TaxiPathRow {
                id: s.id,
                from_taxi_node: s.from_taxi_node,
                to_taxi_node: s.to_taxi_node,
                cost: s.cost,
            }).collect(),
        }
    }
