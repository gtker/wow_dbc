...
impl From<i8> for DurabilityQualityKey {
    fn from(v: i8) -> Self {
        Self::new(v.into())
    }
}

impl From<i16> for DurabilityQualityKey {
    fn from(v: i16) -> Self {
        Self::new(v.into())
    }
}

impl From<i32> for DurabilityQualityKey {
    fn from(v: i32) -> Self {
        Self::new(v)
    }
}

impl TryFrom<u32> for DurabilityQualityKey {
    type Error = u32;
...
mod test {
    ...
    fn durability_quality() {
        let mut file = File::open("../tbc-dbc/DurabilityQuality.dbc").expect("Failed to open DBC file");
