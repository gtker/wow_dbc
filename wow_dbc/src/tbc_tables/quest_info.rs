...
impl DbcTable for QuestInfo {
    ...
    fn filename() -> &'static str { "QuestInfo.dbc" }
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            // id: primary_key (QuestInfo) int32
            let id = QuestInfoKey::new(crate::util::read_i32_le(chunk)?);

            // info_name_lang: string_ref_loc (Extended)
            let info_name_lang = crate::util::read_extended_localized_string(chunk, &string_block)?;


            rows.push(QuestInfoRow {
                id,
                info_name_lang,
            });
        }
    ...
    fn write(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            ...
            // info_name_lang: string_ref_loc (Extended)
            b.write_all(&row.info_name_lang.string_indices_as_array(&mut string_index))?;
...
impl QuestInfo {
    fn write_string_block(&self, b: &mut impl Write) -> Result<(), std::io::Error> {
        ...
        for row in &self.rows {
            row.info_name_lang.string_block_as_array(b)?;
        }
    ...
    fn string_block_size(&self) -> u32 {
        let mut sum = 1;
        for row in &self.rows {
            sum += row.info_name_lang.string_block_size();
        }
...
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct ConstQuestInfo<const S: usize> {
    pub rows: [ConstQuestInfoRow; S],
}

impl<const S: usize> ConstQuestInfo<S> {
    pub const fn const_read(b: &'static [u8], header: &DbcHeader) -> Self {
        ...
        let mut rows = [
            ConstQuestInfoRow {
                id: QuestInfoKey::new(0),
                info_name_lang: crate::ConstExtendedLocalizedString::empty(),
            }
        ; S];
        ...
        while i < S {
            // id: primary_key (QuestInfo) int32
            let id = QuestInfoKey::new(i32::from_le_bytes([b[b_offset + 0], b[b_offset + 1], b[b_offset + 2], b[b_offset + 3]]));
            b_offset += 4;

            // info_name_lang: string_ref_loc (Extended)
            let info_name_lang = ConstExtendedLocalizedString::new(
...
pub struct ConstQuestInfoRow {
    pub id: QuestInfoKey,
    pub info_name_lang: ConstExtendedLocalizedString,
}
