...
impl DbcTable for gtOCTRegenHP {
    ...
    fn read(b: &mut impl std::io::Read) -> Result<Self, crate::DbcError> {
        ...
        for mut chunk in r.chunks(header.record_size as usize) {
            ...
            rows.push(gtOCTRegenHPRow {
                data,
            });
        }

        Ok(gtOCTRegenHP { rows, })
    }
...
pub struct ConstgtOCTRegenHP<const S: usize> {
    pub rows: [gtOCTRegenHPRow; S],
}

impl<const S: usize> ConstgtOCTRegenHP<S> {
    ...
    pub fn to_owned(&self) -> gtOCTRegenHP {
        gtOCTRegenHP {
            rows: self.rows.iter().map(|s| gtOCTRegenHPRow {
                data: s.data,
            }).collect(),
        }
    }
